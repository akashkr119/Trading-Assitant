...
def _render_live_selected_coin(
    ...
    st.caption(
        ...
        "market indicators update every 5 seconds · trade plan changes only on a new scan"
    )
    ...
    st.markdown("#### 🚨 Active Trading Plan")
    if candidate.score >= 75:
        alert_label = "BUY ALERT" if candidate.direction == "LONG" else "SELL ALERT"
        alert_text = "🟢" if candidate.direction == "LONG" else "🔴"
        if candidate.direction == "LONG":
            st.success(
                f"{alert_text} LIVE BUY ALERT — {candidate.symbol} "
                f"at locked entry {candidate.entry:.8g}"
            )
        else:
            st.error(
                f"{alert_text} LIVE SELL ALERT — {candidate.symbol} "
                f"at locked entry {candidate.entry:.8g}"
            )
        st.caption(
            f"Signal generated by the scan. {alert_label} remains fixed until a new scan "
...
if scan_clicked:
    with st.spinner("Scanning crypto pairs and ranking intraday setups..."):
        ...
        # A new scan explicitly replaces the previous trade plans.
        st.session_state.crypto_selected_symbol = None
...
if candidates:
    ...
    st.caption(
        "Live indicators update automatically. Entry, stop and targets are locked to "
        "the latest scan and change only after you run another scan."
    )
