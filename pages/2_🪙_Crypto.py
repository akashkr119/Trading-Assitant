...
def _render_live_selected_coin(selected_symbol: str, locked_candidate: CryptoCandidate) -> None:
    ...
    st.caption(f"{passed}/{total} live core conditions confirmed · 1H confirmation shown separately")
    ...
    if candidate.score >= 75:
        if candidate.direction == "LONG":
            st.success(f"🟢 LIVE BUY ALERT — {candidate.symbol} · locked entry {candidate.entry:.8g}")
        else:
            st.error(f"🔴 LIVE SELL ALERT — {candidate.symbol} · locked entry {candidate.entry:.8g}")
        st.caption("Signal generated by the scan. This trade plan remains fixed until a new scan creates a new plan.")
    else:
        st.warning(f"🟡 NEAR SETUP — {candidate.symbol} · waiting for confirmation · scan entry {candidate.entry:.8g}")
    ...
    st.markdown(f"**Current price vs locked entry:** {entry_status} · {entry_distance:.2f}% from scan entry")

    risk_cols = st.columns(2)
    with risk_cols[0]:
        section_header("⚠️ Invalidation")
        st.warning(f"{candidate.direction} invalid if price closes beyond {candidate.stop_loss:.8g}")
    with risk_cols[1]:
        section_header("🔄 Reversal Watch")
        opposite = "SHORT" if candidate.direction == "LONG" else "LONG"
        st.info(f"Watch for {opposite} confirmation if the 5m/15m trend flips and MACD confirms the opposite direction.")


section_header("⚡ Scanner")
scan_col, limit_col = st.columns([3, 1])
with scan_col:
    scan_clicked = st.button("🔎 Scan crypto intraday opportunities", type="primary", use_container_width=True)
with limit_col:
    scan_limit = st.selectbox("Candidates", [5, 10], index=0)

if scan_clicked:
    with st.spinner("Scanning crypto pairs and ranking intraday setups..."):
        st.session_state.crypto_candidates = scanner.scan(datetime.now(timezone.utc), limit=scan_limit)
...
if candidates:
    ...
    selected_symbol = st.selectbox("🎯 Select a Coin to Trade", candidate_options, index=0, key="crypto_selected_symbol")
    selected_candidate = next((item for item in candidates if item.symbol == selected_symbol), None)
    if selected_candidate is not None:
        st.caption("Live indicators update automatically. Entry, stop and targets remain locked to the latest scan.")
...
st.info("Crypto Swing is the next module. Crypto Intraday remains independent while its live scanner is validated.")
